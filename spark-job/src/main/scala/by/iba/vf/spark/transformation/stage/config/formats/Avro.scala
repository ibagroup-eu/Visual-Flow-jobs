...
object Avro extends GenericFile {
  private val fieldAvroSchema = "avroSchema"

  override def buildConnectionOptions(config: Node): Map[String, String] = {
    val avroSchema = config.value.get(fieldAvroSchema)

    var configMap = super.buildConnectionOptions(config)
    avroSchema.foreach { configMap += "avroSchema" -> _ }

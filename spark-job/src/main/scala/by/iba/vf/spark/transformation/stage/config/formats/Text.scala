...
object Text extends GenericFile {
  private val fieldWholeText = "wholetext"

  override def buildConnectionOptions(config: Node): Map[String, String] = {
    val wholeText = config.value.get(fieldWholeText)

    var configMap = super.buildConnectionOptions(config)
    wholeText.foreach { configMap += "wholetext" -> _ }
